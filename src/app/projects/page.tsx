...
const Projects = () => {
    ...
    return (
        <section id = "projects" className="mt-20 px-40">
            <h1 className="text-center mt-10 text-4xl font-extrabold">
                Here are some of my works
            </h1>
            <div className="mt-10 grid grid-cols-1 gap-6 px-20">
                {projects.map((project) => (
                    <Card key={project.id} title={project.title} description={project.description} imgSrc={project.imgSrc} reversed={project.reversed} icons={project.icons} />
                ))}
            </div>
        </section>
    );
